...
class TicketCreateSerializer(
    ...
    def validate_payment_method_id(
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Método de pago no válido o no pertenece al usuario"
            )
    ...
    def validate(self, data):  # otras validaciones
        ...
        if number < 1 or number > raffle.raffle_number_amount:
            raise serializers.ValidationError(
                f"El número debe estar entre 1 y {raffle.raffle_number_amount}"
            )
...
class TicketListSerializer(
    serializers.ModelSerializer
):  # Serializer para listar tickets con información expandida
...
class TicketRefundSerializer(
    serializers.ModelSerializer
):  # Serializer para reembolsar tickets individuales
    class Meta:
        model = Ticket
        fields = ["id"]
        read_only_fields = ["id"]
