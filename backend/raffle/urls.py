...
urlpatterns = [
    ...
    path('<int:pk>/delete/', RaffleSoftDeleteView.as_view(), name='raffle-delete'), # PATCH - Soft delete (con reembolsos)
    path('<int:pk>/admin-cancel/', AdminRaffleCancelView.as_view(), name='admin-raffle-cancel'), # PATCH - Cancelación administrativa
    path('<int:pk>/draw/', RaffleDrawView.as_view(), name='raffle-draw'),          # PATCH - Ejecutar sorteo
    path('<int:pk>/available/', AvailableNumbersView.as_view(), name='available-numbers'), # GET - Números disponibles
