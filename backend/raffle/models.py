...
def raffle_image_upload_path(instance, filename):
    ...
    clean_name = "".join(
        c for c in filename.split(".")[0] if c.isalnum() or c in (" ", "-", "_")
    ).rstrip()
...
class Raffle(models.Model):
    raffle_name = models.CharField(
        max_length=100, verbose_name="Nombre de la rifa", help_text="Nombre de la rifa"
    )
    ...
    raffle_prize_type = models.ForeignKey(
        PrizeType, on_delete=models.RESTRICT, verbose_name="Tipo de premio"
    )
    raffle_state = models.ForeignKey(
        StateRaffle, on_delete=models.RESTRICT, verbose_name="Estado de la rifa"
    )
    ...
    def clean(self):  # Validaciones personalizadas
        ...
        if self.raffle_start_date and self.raffle_draw_date:
            if self.raffle_start_date >= self.raffle_draw_date:
                errors.append(
                    "La fecha de inicio debe ser anterior a la fecha del sorteo"
                )

        # Solo validar fecha del pasado si la rifa está activa y no ha sido procesada
        if (
            self.pk
            and self.raffle_draw_date
            and self.raffle_draw_date < timezone.now()
            and not self.raffle_winner
            and self._is_in_active_state()
            and not getattr(self, "_allow_past_date", False)
        ):  # Flag para permitir cambios administrativos
            errors.append("La fecha del sorteo no puede ser en el pasado")

        if (
            self.raffle_minimum_numbers_sold
            and self.raffle_number_amount
            and self.raffle_minimum_numbers_sold > self.raffle_number_amount
        ):
            errors.append(
                "El mínimo de números para sortear no puede ser mayor al total de números disponibles"
            )
    ...
    def save(
        self, *args, **kwargs
    ):  # Asignar estado activo por defecto solo si la rifa es nueva
    ...
    def is_active_for_sales(self):  # Verifica si la rifa está activa para ventas
        now = timezone.now()
        return (
            self._is_in_active_state()
            and self.raffle_start_date <= now < self.raffle_draw_date
            and self.numbers_available > 0
            and not self.raffle_winner  # No ha sido sorteada
        )
    ...
    def is_ready_for_draw(self):  # Verifica si la rifa está lista para ser sorteada
        ...
        return (
            self._is_in_active_state()
            and now >= self.raffle_draw_date
            and self.minimum_reached
            and not self.raffle_winner  # No ha sido sorteada aún
        )
    ...
    def _get_admin_payment_method(self):
        ...
        if not admin_user:
            raise ValidationError(
                "No existe usuario admin con identificación 0000000000 para cuenta conjunta"
            )
        admin_account = PaymentMethod.objects.filter(
            user=admin_user, payment_method_is_active=True
        ).first()
    ...
    def cancel_raffle_and_refund(self, admin_reason=None):
        ...
        cancelled_state = (
            StateRaffle.objects.filter(state_raffle_code__iexact="CAN").first()
            or StateRaffle.objects.filter(state_raffle_name__icontains="cancel").first()
        )
    ...
    def can_execute_draw(self):  # Verifica si se puede ejecutar el sorteo
        ...
        if not self.minimum_reached:
            ...
            return (
                False,
                f"No se alcanzó el mínimo. Faltan {remaining} números por vender",
            )
    ...
    def execute_raffle_draw(self):  # Ejacuta el sorteo de la rifa
        ...
        if (
            self.raffle_prize_type.prize_type_code.upper() == "DIN"
            or self.raffle_prize_type.prize_type_name.upper() == "DINERO"
        ):
            if gains < 0:
                deficit = abs(gains)
                if not self.raffle_creator_payment_method.has_sufficient_balance(
                    deficit
                ):
                    ...
                    for ticket in sold_tickets:
                        try:
                            if admin_account.has_sufficient_balance(
                                self.raffle_number_price
                            ):
                                admin_account.deduct_balance(self.raffle_number_price)
                                ticket.payment_method.add_balance(
                                    self.raffle_number_price
                                )
                        except Exception:
                            pass
                    ...
                    cancelled_state = (
                        StateRaffle.objects.filter(
                            state_raffle_code__iexact="CAN"
                        ).first()
                        or StateRaffle.objects.filter(
        ...
        # Usar secrets.SystemRandom() para sorteo criptográficamente seguro
        ...
        if (
            ...
            try:
                ...
                raise ValueError(
                    f"No existe cuenta conjunta de admin para el sorteo: {e}"
                )
            ...
            if admin_account.has_sufficient_balance(self.raffle_prize_amount):
                ...
                raise ValueError(
                    "La cuenta conjunta no tiene saldo suficiente para el premio"
                )

            # Entregar ganancias al creador desde la cuenta conjunta (solo si son positivas)
            if (
                self.raffle_creator_payment_method
                and gains > 0
                and admin_account.has_sufficient_balance(gains)
            ):
        ...
        return {  # Resultados del sorteo
            ...
            "winner_payment_method": str(
                winner_ticket.payment_method.payment_method_type
            ),
    ...
    def _change_state_to_sorted(self):  # Cambia el estado de la rifa a sorteada
        ...
        try:
            sorted_state = (
                StateRaffle.objects.filter(state_raffle_code__iexact="SOR").first()
                or StateRaffle.objects.filter(
