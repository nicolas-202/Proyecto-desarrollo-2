...
class PaymentMethodViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return PaymentMethod.objects.filter(
            user=self.request.user, payment_method_is_active=True
        )
    ...
    def check_balance(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"error": "Monto inválido"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {
                ...
                "message": (
                    "Saldo suficiente" if has_sufficient else "Saldo insuficiente"
                ),
