...
from .models import DocumentType, Gender, PaymentMethod, PaymentMethodType
...
class PaymentMethodSerializer(serializers.ModelSerializer):
    ...
    payment_method_type_name = serializers.CharField(
        source="payment_method_type.payment_method_type_name", read_only=True
    )
    ...
    class Meta:
        ...
        read_only_fields = ("id", "user", "last_digits", "created_at", "updated_at")
        extra_kwargs = {
            "paymenth_method_holder_name": {
                "required": False,  # No obligatorio para actualizaciones parciales
                "allow_blank": False,
    ...
    def validate_paymenth_method_holder_name(self, value):
        ...
        if len(cleaned_value) > 100:
            raise serializers.ValidationError(
                "El nombre del titular no puede exceder 100 caracteres."
            )
