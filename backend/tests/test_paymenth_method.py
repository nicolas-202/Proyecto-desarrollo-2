...
class PaymentMethodAPITestCase(APITestCase):
    ...
    def get_valid_payment_method_data(self):
        ...
        return {
            ...
            'paymenth_method_expiration_date': (date.today() + timedelta(days=365)).isoformat()
        }
    ...
    def test_admin_can_see_all_methods(self):
        """Admin puede ver todos los métodos"""
        self.client.force_authenticate(user=self.admin_user)
        
        # Crear métodos para diferentes usuarios
        self.create_payment_method(self.regular_user)
        self.create_payment_method(self.other_user)
        
        response = self.client.get(self.payment_method_list_url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)  # Ve todos
    ...
    def test_create_with_past_expiration_date(self):
        ...
        self.client.force_authenticate(user=self.regular_user)
        data = self.get_valid_payment_method_data()
        data['paymenth_method_expiration_date'] = (date.today() - timedelta(days=1)).isoformat()
    ...
    def test_holder_name_is_title_cased(self):
        ...
        response = self.client.post(self.payment_method_list_url, data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['paymenth_method_holder_name'], 'Juan Carlos Pérez')
    ...
    def test_create_with_card_number_with_spaces_success(self):
        ...
        self.assertTrue(payment_method.check_card_number('1234567890123456'))  # Sin espacios
    ...
    def test_admin_cannot_update_payment_method(self):
        """Admin no puede actualizar métodos de pago"""
        self.client.force_authenticate(user=self.admin_user)
        payment_method = self.create_payment_method(self.regular_user)
        
        url = reverse('payment-method-detail', args=[payment_method.pk])
        data = {'paymenth_method_holder_name': 'Admin Update'}
    ...
    def test_admin_can_update_own_payment_method(self):
        ...
        data = {
            ...
            'paymenth_method_expiration_date': (date.today() + timedelta(days=730)).isoformat()
        }
        
        response = self.client.patch(url, data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['paymenth_method_holder_name'], 'Admin Updated Name')
