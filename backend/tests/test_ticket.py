...
class TicketSystemTestCase(APITestCase):
    ...
    def test_ticket_purchase_success(self):
        ...
        self.assertEqual(
            self.admin_payment_method.payment_method_balance, Decimal("10.00")
        )
    ...
    def test_raffle_available_numbers(self):
        ...
        Ticket.purchase_ticket(
            self.participant1, self.main_raffle, 10, self.payment_method1
        )
        Ticket.purchase_ticket(
    ...
    def test_raffle_draw_success(self):
        ...
        self.admin_payment_method.refresh_from_db()
        self.assertEqual(
            self.admin_payment_method.payment_method_balance, Decimal("220.00")
        )
        ...
        self.assertEqual(
            self.organizer_payment_method.payment_method_balance, Decimal("9840.00")
        )
    ...
    def test_organizer_insufficient_balance_for_deficit(self):
        ...
        self.assertEqual(
            self.payment_method1.payment_method_balance,
            initial_participant1_balance - Decimal("10.00"),
        )
        self.assertEqual(
            self.payment_method2.payment_method_balance,
            initial_participant2_balance - Decimal("10.00"),
        )
        ...
        self.assertIn(
            "no tiene saldo suficiente para cubrir el déficit", str(context.exception)
        )
        ...
        self.payment_method2.refresh_from_db()
        self.assertEqual(
            self.payment_method1.payment_method_balance, initial_participant1_balance
        )
        self.assertEqual(
            self.payment_method2.payment_method_balance, initial_participant2_balance
        )
    ...
    def test_only_organizer_can_cancel(self):
        ...
        with self.assertRaises(ValidationError) as context:
            self.main_raffle.soft_delete_and_refund(
                self.participant1
            )  # No es organizador
    ...
    def test_admin_balance_after_raffle_draw(self):
        ...
        Ticket.purchase_ticket(
            self.admin_user, self.main_raffle, 3, self.admin_payment_method
        )
        ...
        self.admin_payment_method.refresh_from_db()
        self.assertEqual(
            self.admin_payment_method.payment_method_balance, Decimal("230.00")
        )
        ...
        # 3. Cuenta conjunta paga premio: 380.00 - 180.00 = 200.00
        #
        # NOTA: Si el ganador es el admin, el premio va y vuelve a la misma cuenta
        # por lo que el saldo final sería 380.00 (recibe déficit pero no paga premio neto)
        self.admin_payment_method.refresh_from_db()

        # Verificar el saldo según quién ganó
        winner = self.main_raffle.raffle_winner
        if winner == self.admin_user:
            # Si gana el admin, el premio va y vuelve a la misma cuenta (380)
            self.assertEqual(
                self.admin_payment_method.payment_method_balance, Decimal("380.00")
            )
        else:
            # Si gana otro participante, se paga el premio normalmente (200)
            self.assertEqual(
                self.admin_payment_method.payment_method_balance, Decimal("200.00")
            )
        ...
        self.organizer_payment_method.refresh_from_db()
        self.assertEqual(
            self.organizer_payment_method.payment_method_balance, Decimal("9850.00")
        )
