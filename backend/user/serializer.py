...
from userInfo.serializer import GenderSerializer, DocumentTypeSerializer
...
class RegisterUserSerializer(serializers.ModelSerializer): 
    ...
    def validate_document_number(self, value):
        ...
        if not str(value).isdigit():
            raise serializers.ValidationError("El número de documento debe ser un valor numérico.")
...
class UserBasicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id', 'email', 'first_name', 'last_name', 'full_name', 'phone_number', 'rating')
...
class AdminUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id','email','full_name','is_active','is_admin', 'document_number', 'document_type')
    ...
    def validate(self, data):
        ...
        if not is_admin and instance:
            ...
            if not active_admins.exists():
                raise serializers.ValidationError(f"No se puede desactivar el último administrador (ID: {instance.id}).")

        is_active = data.get('is_active', instance.is_active if instance else True)
        if not is_active and instance and current_user and instance.id == current_user.id:
