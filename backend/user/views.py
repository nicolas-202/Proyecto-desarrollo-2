    ...
    def perform_update(self, serializer):
        ...
        if (
            ...
            raise serializer.ValidationError(
                "No puedes desactivar tu propia cuenta como administrador"
            )
...
class DeleteAccountViewSet(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            hasattr(request.user, "organized_raffles")
            and request.user.organized_raffles.filter(status="active").exists()
        ):
            return Response(
                {
        ...
        serializer.save()
        return Response(
            {"message": "Cuenta desactivada exitosamente"}, status=status.HTTP_200_OK
        )
