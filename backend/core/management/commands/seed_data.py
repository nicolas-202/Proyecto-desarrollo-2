...
class Command(BaseCommand):
    ...
    def create_document_types(self):
        ...
        document_types = [
            (
                ...
                "Documento de identificación para ciudadanos colombianos",
            ),
        ]
        for code, name, desc in document_types:
            ...
            if created:
                self.stdout.write(
                    f"  ✓ Tipo documento creado: {doc_type.document_type_name}"
                )
    ...
    def create_payment_method_types(self):
        ...
        payment_types = [("TDEB", "Tarjeta Débito", "Tarjeta débito bancaria")]
        ...
        for code, name, desc in payment_types:
            ...
            if created:
                self.stdout.write(
                    f"  ✓ Tipo pago creado: {payment_type.payment_method_type_name}"
                )
    ...
    def create_prize_types(self):
        ...
        prize_types = [("DIN", "Dinero", "Premio en dinero en efectivo")]
        ...
        for code, name, desc in prize_types:
            ...
            if created:
                self.stdout.write(
                    f"  ✓ Tipo premio creado: {prize_type.prize_type_name}"
                )
    ...
    def create_admin_user_payment_method(self):
        ...
        try:
            ...
            )  # 10 ceros según tu modelo
            debit_card_type = PaymentMethodType.objects.get(
                payment_method_type_code="TDEB"
            )
        except User.DoesNotExist:
            self.stdout.write(
                self.style.ERROR(
                    "  ❌ Error: Usuario admin no encontrado (debe tener documento 0000000000)"
                )
            )
            return
            ...
            self.stdout.write(
                self.style.ERROR(
                    "  ❌ Error: Tipo de método de pago TDEB no encontrado"
                )
            )
            return

        # Verificar si ya existe
        if admin_user.payment_methods.filter(
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"  ✓ Método de pago creado para admin - Saldo: ${payment_method.payment_method_balance:,.0f}"
            )
        )
    ...
    def create_user_1(self):
        ...
        if not User.objects.filter(email=user_email).exists():
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"  🔑 Password: usuario123 (¡CAMBIAR EN PRODUCCIÓN!)"
                )
            )
        else:
    ...
    def create_user1_payment_method(self):
        ...
        # Verificar si ya existe
        if regular_user.payment_methods.filter(
            payment_method_type=debit_card_type
        ).exists():
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"  ✓ Método de pago creado para usuario regular - Saldo: ${payment_method.payment_method_balance:,.0f}"
            )
        )
